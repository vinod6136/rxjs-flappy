	...
	scan<any, any>(
		(acc) =>
			(acc.length < 2 ? [...acc, createPipes(gameSize)] : acc)
				...
				.map((cols: any[]) =>
					cols.map((e: { x: any; y: number }) => gamePipe(e.x, e.y - 1))
				),
...
const updateGame = (bird: string | number, pipes: any[]) =>
	((game:any) => (
		pipes.forEach((col: any[]) =>
			col.forEach(
				(v: { x: string | number; y: string | number }) => (game[v.x][v.y] = 2)
			)
		),
		(game[bird][0] = 1),
		game
...
const valueOnCollisionFor = (pipes: { checked: boolean }[][]) => ({
	when: (predicate: any) =>
			...
			(state, [bird, pipes]) => ({
				...
				lives:
					...
					valueOnCollisionFor(pipes).when(
						pipes.some((c: any[]) =>
							c.some((c: { y: number; x: any }) => c.y === 0 && c.x === bird)
						)
					),
				score:
